...
const MESSAGES = {
  zh: {
    ...
    healthTitle: "查看更新状态", healthMode: "模式", healthSnapshotMode: "后台采集 · 前端读取快照", healthMarketDate: "MSTR 行情日期", healthSnapshotCreated: "快照生成（UTC+8）", healthReadCheck: "最近快照检查（UTC+8）", healthReadStatus: "快照读取结果", healthReadOk: "读取成功", healthReadFailed: "读取失败 · 保留原读数", healthReadError: "本次读取失败原因", healthBackendCheck: "后台最近检查", healthBackendSuccess: "后台最近成功采集", healthBackendError: "后台最近失败原因", healthNotConnected: "独立记录暂未接入", healthUnknown: "暂无可核验记录", healthReadOnlyNote: "读取快照成功不代表上游采集成功；快照生成时间也不是后台最近检查时间。",
  ...
  en: {
    ...
    healthTitle: "View update status", healthMode: "Mode", healthSnapshotMode: "Background collection · Snapshot read", healthMarketDate: "MSTR market date", healthSnapshotCreated: "Snapshot created (UTC+8)", healthReadCheck: "Latest snapshot check (UTC+8)", healthReadStatus: "Snapshot read result", healthReadOk: "Read succeeded", healthReadFailed: "Read failed · Previous reading retained", healthReadError: "Snapshot read failure", healthBackendCheck: "Latest backend check", healthBackendSuccess: "Latest successful collection", healthBackendError: "Latest backend failure", healthNotConnected: "Independent records not connected yet", healthUnknown: "No verified record", healthReadOnlyNote: "Reading a snapshot does not verify upstream collection. Snapshot creation time is not the latest backend check time.",
