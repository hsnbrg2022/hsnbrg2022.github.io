...
const MESSAGES = {
  zh: {
    ...
    etfMaintenanceTitle: "维护 BTC ETF 资金流", etfMaintenanceNote: "只需填写交易日期和当日总净流量，系统将自动计算连续天数、累计金额和信号状态。", etfTradeDate: "交易日期", etfNetFlow: "当日总净流量（百万美元）", etfNetFlowHelp: "正数表示净流入，负数表示净流出", etfManualSource: "人工核对来源", etfRecentHistory: "最近记录", etfPublishHint: "保存后将同步公开版快照；上传 etf-flows.json 和 dashboard.json 即可发布给访客。", etfSave: "保存并同步", etfSaved: "ETF 数据已保存，公开版快照已同步", etfLoading: "正在读取 ETF 历史…",
    etfLocalNote: "填写交易日期和当日总净流量；保存会更新本地看板，并同步生成 GitHub Pages 发布数据。", etfBrowserNote: "填写交易日期和当日总净流量；公开版的修改只保存在当前浏览器，不会影响其他访客。", etfLocalPolicy: "全局发布方式：本地保存后，上传 dashboard.json 与 etf-flows.json 到 GitHub 根目录。", etfBrowserPolicy: "浏览器本地数据：不会写回 GitHub，不会同步到其他设备或访客。", etfSavedBrowser: "ETF 数据已保存到当前浏览器，不影响其他访客", etfResetBrowser: "已恢复 GitHub 发布的 ETF 数据", etfLoadFailed: "无法读取 ETF 历史", browserOnlyShort: "当前浏览器手工数据",
  ...
  en: {
    ...
    etfMaintenanceTitle: "Maintain BTC ETF flows", etfMaintenanceNote: "Enter the trading date and daily total net flow. Streaks, cumulative flow and signal status are calculated automatically.", etfTradeDate: "Trading date", etfNetFlow: "Daily total net flow (USD millions)", etfNetFlowHelp: "Positive means net inflow; negative means net outflow", etfManualSource: "Manual verification source", etfRecentHistory: "Recent records", etfPublishHint: "Saving syncs the public snapshot. Upload etf-flows.json and dashboard.json to publish it for visitors.", etfSave: "Save and sync", etfSaved: "ETF flow saved and the public snapshot is synced", etfLoading: "Loading ETF history…",
    etfLocalNote: "Enter the trading date and daily total net flow. Saving updates the local dashboard and generates the GitHub Pages data files.", etfBrowserNote: "Enter the trading date and daily total net flow. Public-site changes stay only in this browser and do not affect other visitors.", etfLocalPolicy: "Global publishing: after saving locally, upload dashboard.json and etf-flows.json to the GitHub repository root.", etfBrowserPolicy: "Browser-local data: it is not written back to GitHub and does not sync to other devices or visitors.", etfSavedBrowser: "ETF flows saved in this browser only; other visitors are unaffected", etfResetBrowser: "Published ETF data restored", etfLoadFailed: "Unable to load ETF history", browserOnlyShort: "Browser-local manual data",
